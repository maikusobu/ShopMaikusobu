...
export const ModalAddPaymentContext = createContext<ModalContextType | null>(null);
function PaymentProvider({ children }: { children: React.ReactNode }) {
	...
	const form = useForm<UserPaymentValues>({
		...
		validate: {
			payment_type: (v) => (isValidPaymentType(v) ? null : 'Invalid payment type!'),
			card_number: (v) => (isValidCardNumber(v) ? null : 'Please typing the valid'),
			expire: (v) => (isValidExpireDate(v as Date) ? null : 'Invalid expire date!'),
		},
	...
	const formatCreditCardNumber = (value: string): string => {
		...
		if (valueTrim.length <= 8) return valueTrim.slice(0, 4) + ' ' + valueTrim.slice(4, 8);
		if (valueTrim.length <= 12)
			return valueTrim.slice(0, 4) + ' ' + valueTrim.slice(4, 8) + ' ' + valueTrim.slice(8, 12);
	...
	const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
		...
		createUserPayment(formData)
			.unwrap()
			.then((data) => {
				if (value === true || import.meta.env.VITE_DEFAULT === userData?.idDefaultPayment)
					updateUser({
						id: auth.id,
			...
			>
				<form className={classes.formContainer} onSubmit={handleSubmit}>
					...
					<Select
						...
						onChange={(value) => {
							if (value)
								form.setFieldValue('payment_type', value as 'credit' | 'debit' | 'paypal' | 'bank');
					...
					<DateInput
						...
						popoverProps={{
							styles: {
								dropdown: { scale: '0.9', left: '0px !important', top: '0px !important' },
							},
						}}
						required
					/>
